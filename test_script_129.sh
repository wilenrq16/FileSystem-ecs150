...
# create 33 files
for i in $(seq -w 1 129); do echo hello how are youuuuuuU!!!!!!!!!!!!!!!! > file${i}; done

# get fs_info from reference lib
for i in $(seq -w 1 129); do ./fs_ref.x add disk.fs file${i}; done
./fs_ref.x ls disk.fs >ref.stdout 2>ref.stderr

for i in $(seq -w 1 129); do ./fs_ref.x rm disk.fs file${i}; done

# get fs_info from my lib
for i in $(seq -w 1 129); do ./test_fs.x add disk.fs file${i}; done
./test_fs.x ls disk.fs >lib.stdout 2>lib.stderr
for i in $(seq -w 1 129); do ./test_fs.x rm disk.fs file${i}; done
...
for i in $(seq -w 1 129); do rm file${i}; done